...
func main() {
	...
	for i := 0; i <= len(split)-3; i += 3 {
		...
		second := split[i+1]
		third := split[i+2]
		diff := diffMap(strToMap(first), strToMap(second))
		diff = diffMap(strToMap(strings.Join(diff, "")), strToMap(third))
		c := count(diff)
		fmt.Println(diff, c)
		result += c
	}
...
func diffMap(a map[string]int, b map[string]int) []string {
...
func count(input []string) int {
	...
	for _, str := range input {
		if str[0] <= 'z' && str[0] >= 'a' {
			result += int(str[0]) - 'a' + 1
		} else {
			result += int(str[0]) - 'A' + 27
		}
	}
	return result
}
